...
func build_send_cheer_string(amount_min, amount_max, source):
	...
	if str(amount_min) != str(amount_max):
		if str(amount_max) == "all":
			...
			amount_str = "%s-%s" % [int(amount_min), int(amount_max)]
...
func build_choose_cards_string(from_zone, to_zone, amount_min, amount_max,
	...
	var amount_str = "%s" % int(amount_min)
	if amount_min != amount_max:
					amount_str = "%s-%s" % [int(amount_min), int(amount_max)]
		...
		match condition["condition"]:
			...
			"cards_in_hand":
				...
				if amount_min == 0 and amount_max == 0:
					...
					if amount_min == -1:
						amount_str += "at most %s" % int(amount_max)
					elif amount_max == -1:
			...
			"has_stacked_holomem":
				if "amount_min" in condition:
					...
					text += tr("CONDITION__HAS_STACKED_HOLOMEM_AMOUNT_MIN").format({Amount=int(condition["amount_min"])})
				else:
					text += ""
			"holomem_in_archive":
				var amount_str = ""
				if "amount_min" in condition and "amount_max" not in condition:
					amount_str += "%s+" % int(condition["amount_min"])
			...
			"performance_target_has_damage_over_hp":
				text += "Damage exceeds hp by %s: " % [int(condition["amount"])]
			"performer_is_center":
				text += tr("Center:") + " "
			"performer_is_collab":
				text += tr("Collab:") + " "
			...
			"self_has_cheer_color":
				var amount_str = int(condition["amount_min"])
...
func get_effect_text(effect):
	...
	match effect_type:
		...
		"bonus_cheer":
			text += "Treat as %s %s Cheer for Arts." % [int(effect["amount"]), effect["color"]]
		"bonus_hp":
			text += "+%s HP." % [int(effect["amount"])]
		...
		"choose_cards":
			...
			if "look_at" in effect:
				...
				if look_at == -1:
					...
					look_at = tr("the top %s") % int(look_at)
			...
			var choose_str = build_choose_cards_string(
				...
				effect["amount_min"],
				effect["amount_max"],
				effect.get("remaining_cards_action", "NULL"),
				effect,
		...
		"deal_damage":
			...
			var amount_str = str(int(effect["amount"]))
		...
		"force_die_result":
			text += tr("Set next die result to %s.") % [int(effect["die_result"])]
		"generate_holopower":
			text += "Generate %s Holopower." % [int(effect["amount"])]
		"go_first":
		...
		"move_cheer_between_holomems":
			...
			text += "Move %s Cheer between your Holomems%s." % [int(amount), limitation_str]
		"order_cards":
		...
		"performance_life_lost_increase":
			text += "Increase life lost by %s." % [int(effect["amount"])]
		"place_holomem":
		...
		"power_boost":
			...
			text += "+%s Power%s." % [int(effect["amount"]), multiplier_str]
		"power_boost_per_all_fans":
			text += "+%s Power per each Fan attached to your Holomems." % [int(effect["amount"])]
		"power_boost_per_all_mascots":
			#Example: `+20 Power per each Mascot attached to your Holomems.`
			text += tr("EFFECT__POWER_BOOST_PER_ALL_MASCOTS").format({Amount=int(effect["amount"])})
		"power_boost_per_archived_holomem":
			text += "+%s Power per archived Holomem." % [int(effect["amount"])]
		"power_boost_per_attached_cheer":
			var up_to_str = ""
			if "limit" in effect:
				up_to_str = " (up to %sx)" % effect["limit"]
			text += "+%s Power per attached Cheer%s." % [int(effect["amount"]), up_to_str]
		"power_boost_per_backstage":
			text += "+%s Power per Back member." % [int(effect["amount"])]
		"power_boost_per_holomem":
			...
			if "limit" in effect:
				up_to_str = " (up to %sx)" % effect["limit"]
			text += "+%s Power per %s%sHolomem%s." % [int(effect["amount"]), tag_str, exclude_str, up_to_str]
		"power_boost_per_revealed_card":
			...
			text += tr("EFFECT__POWER_BOOST_PER_STACKED").format({Amount=int(amount), Limitation=limitation_str})
		"power_boost_per_stacked":
			text += tr("+%s Power per stacked Holomem.") % [int(effect["amount"])]
		"power_boost_per_played_support":
			var support_type = tr(effect["support_sub_type"])
			text += tr("+%s Power per %s played this turn.") % [int(effect["amount"]), support_type]
		...
		"reduce_damage":
			if str(effect["amount"]) == "all":
				...
				text += "Reduce damage by %s." % [int(effect["amount"])]
		"reduce_required_archive_count":
			var amount = effect["amount"]
			var cheer_color = effect.get("cheer_color")
			if cheer_color:
				text += "Archive %s less %s cheer from Holomem" % [int(amount), "/".join(cheer_color.map(get_color_string))]
			else:
				text += "Archive %s less from Hand" % [int(amount)]
		"repeat_art":
		...
		"restore_hp":
			...
			text += "Restore %s HP %s%s" % [int(amount_str), target_str, limitation_str]
		"restore_hp_INTERNAL":
			text += "Restore HP."
		...
		"reveal_top_deck":
			...
			var amount_str = "%s " % int(amount) if amount else ""
		...
		"set_center_hp":
			if "opponent" in effect and effect["opponent"]:
				text += "Reduce opponent's Center's remaining HP to %s." % [int(effect["amount"])]
			else:
				text += "Reduce your Center's remaining HP to %s." % [int(effect["amount"])]
	...
	match definition["card_type"]:
		"holomem_debut", "holomem_bloom", "holomem_spot":
			...
			if "bloom_level_skip" in definition:
				var bloom_text = "Can bloom directly to level %s" % int(definition["bloom_level_skip"])
			...
			if "mascot_count_limit" in definition:
				...
				mascot_text += tr("EFFECT__MASCOT_COUNT_LIMIT").format({Amount=int(definition["mascot_count_limit"])})
			...
			for art in arts:
				...
				var text = "[b]%s: %s[/b]" % [get_skill_string(art["art_id"]), int(art["power"])]
			...
			data.append({"colors": [], "text": "Baton Pass Cost: %s" % int(definition["baton_cost"])})
			if "down_life_cost" in definition and definition["down_life_cost"] != 1:
				data.append({"colors": [], "text": "Life lost when downed: %s" % int(definition["down_life_cost"])})
		"support":
			var effects = []
				...
				for requirement_name in play_requirements:
					match requirement_name:
						"cheer_to_archive_from_play":
							next_entry["text"] += "Must archive %s Cheer.\n" % [int(play_requirements[requirement_name]["length"])]
					data.append(next_entry)
					next_entry = {
						"colors": [],
						"text": ""
		...
		"oshi":
			for action in definition.get("actions", []):
				...
				if action["limit"] == "once_per_game":
					limit = tr("1/Game")
				var text = "-%s [b]%s[/b] (%s): " % [int(action["cost"]), get_skill_string(action["skill_id"]), limit]
				var effects_text = ""

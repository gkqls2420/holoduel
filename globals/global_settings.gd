...
signal setting_changed_UseWebServer
...
const ClientVersionString : String = "250101.0101" # YYMMDD.HHMM
...
const ShowBigCard = "ShowBigCard"
const UserClientVersion = "UserClientVersion"
const UseWebServer = "UseWebServer"
...
var setting_to_signal_map = {
	...
	UseWebServer: setting_changed_UseWebServer
}
...
# 서버 URL 목록 (우선순위 순서)
const SERVER_URLS = [
	"wss://web-production-db70.up.railway.app/ws",  # Railway 서버 (1순위)
	"ws://127.0.0.1:8000/ws"  # 로컬 서버 (2순위)
]

# 플랫폼 환경 감지
func is_html5_export() -> bool:
	return OS.get_name() == "Web"

func is_android_export() -> bool:
	return OS.get_name() == "Android"

func get_platform_name() -> String:
	return OS.get_name()

var current_server_index = 0  # 현재 사용 중인 서버 인덱스

func get_server_url() -> String:
	# HTML5/Android 환경에서는 Railway 서버만 사용
	if is_html5_export() or is_android_export():
		return SERVER_URLS[0]  # Railway 서버만 사용
	
	# 데스크톱 환경에서는 사용자 설정에 따라 서버 선택
	var use_web_server = get_user_setting(UseWebServer)
	if use_web_server:
		return SERVER_URLS[0]  # Railway 서버
	else:
		return SERVER_URLS[1]  # 로컬 서버

func get_next_server_url() -> String:
	# 다음 서버 URL로 이동
	current_server_index += 1
	if current_server_index >= SERVER_URLS.size():
		current_server_index = 0  # 처음부터 다시 시작
	return get_server_url()

func reset_server_index():
	# 서버 인덱스를 처음으로 리셋
	current_server_index = 0

func get_current_server_name() -> String:
	var use_web_server = get_user_setting(UseWebServer)
	if use_web_server:
		return "Railway"
	else:
...
func has_more_servers() -> bool:
	# HTML5/Android 환경에서는 폴백 비활성화
	if is_html5_export() or is_android_export():
		return false
	
	# 데스크톱 환경에서도 사용자 설정에 따라 폴백 비활성화
	return false

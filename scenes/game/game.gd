...
var selected_cards = []
var current_requirement_details = {}
...
func handle_game_event(event_type, event_data):
	...
	if event_type == Enums.EventType_ObserverCaughtUp:
		...
		return
	if len(full_event_log) > 0 and event_data.get("event_number", 0) <= full_event_log[-1].get("event_number", 0):
		Logger.log_game("Ignoring event %s because it has already been processed" % event_data.get("event_number", 0))
		return
...
func _update_player_stats(player, stats_group):
	...
	player._hand_indicator.text = str(int(player.hand_count))
...
func _is_selection_requirement_met():
	...
	var count_valid = selection_min <= count and count <= selection_max
	
	# Check for requirement_different_colors (two_tone_color_pc)
	if current_requirement_details.get("requirement_different_colors", false):
		if count >= 2:
			# Get colors from selected cards
			var all_colors = []
			for card_graphic in selected_cards:
				var card_definition = CardDatabase.get_card(card_graphic._definition_id)
				if card_definition and "colors" in card_definition:
					for color in card_definition["colors"]:
...
func _on_boost_stat_event(event_data):
	...
	if stat == "damage_prevented":
		...
		_play_transient_icon_message(str(int(amount)), card.get_center_position(), PopupMessage.IconMessageType.Shield, true)
	else:
		_play_popup_message("+%s %s" % [int(amount), Strings.get_stat_string(stat)])
...
func _on_choose_cards_event(event_data):
	...
	if active_player.is_me() and not observer_mode:
		...
		if active_player.are_cards_in_zone_visible(from_zone) and from_zone != "archive":
			...
			current_requirement_details = requirement_details
...
func _change_ui_phase(new_ui_phase : UIPhase):
	...
	if new_ui_phase == UIPhase.UIPhase_WaitingOnServer:
		# Clear requirement details when submitting decision
		current_requirement_details = {}
...
func _on_modify_hp_event(event_data):
	...
	_play_transient_icon_message(str(int(damage_done)), card.get_center_position(), PopupMessage.IconMessageType.Damage, true)
...
func _on_damage_dealt_event(event_data):
	...
	_play_transient_icon_message(str(int(damage)), card.get_center_position(), PopupMessage.IconMessageType.Damage, true)
...
func _on_restore_hp_event(event_data):
	...
	_play_transient_icon_message(str(int(healed_amount)), card.get_center_position(), PopupMessage.IconMessageType.Heart, true)
...
func _on_roll_die_event(event_data):
	...
	_play_popup_message(tr("Rolled die:") + " %s%s" % [int(die_result), rigged_str])
...
func _on_life_damage_dealt(event_data):
	...
	_play_popup_message(tr("DEAL_LIFE_DAMAGE").format({Amount=int(life_lost)}))
...
func _on_card_hovered(_card_id : String, card : CardBase, is_hover : bool):
	if GlobalSettings.get_user_setting(GlobalSettings.ShowBigCard):
